...
double G3(double s) {
  const double st = stack(s);
  return inv_f(1.0-s)*(1.0-s)+R*(st)-R*(1.0-s)+inv_g(s)*s + (1.0-inv_f(st)-inv_g(s))*st;
}

double G4(double a, double b) {
  return inv_f(1.0-b)*(1.0-a)+(1.0-inv_g(a)-inv_f(1.0-b))*stack(b) + inv_g(a)*a + (inv_g(b) - inv_g(a))*max(0.0,0.5+b-1.0+a);
}

double G5(double a, double b) {
  const double st = stack(b);
  return (1.0 - a)*inv_f(1.0-a) + R*st-R*(1.0-a) + a*inv_g(a) + (1.0 - inv_f(stack(b)) - inv_g(a))*st + max(0.0, 0.5 + b - (1.0-a))*(inv_g(b) - inv_g(a));
;
}
